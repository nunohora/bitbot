...
function KrakenClient(key, secret, otp) {
	...
	function rawRequest(url, headers, params, callback) {
		...
		headers['User-Agent'] = 'KrakenClient';
		...
		var req = request.post(options, function(error, data) {
			if (!data || !JSON.parse(data['body']) || JSON.parse(data['body'])['error'].length > 0) {
				callback.call(self, true, null);
			}
			else {
				try {
					callback.call(self, null, JSON.parse(data['body']));
				}
				catch (e) {
					console.log(e);
					callback.call(self, true, null);
